...
// PATCH 4.1: Compute equivalent API cost (in cents) from token usage,
// even when billing_type=subscription_included. Used to populate the
// equivalent_api_cost_cents column so the dashboard can show the real
// dollar value of subscription-billed work without false-triggering
// budgets (cost_cents stays at 0 for subscription rows).
// Pricing in USD per 1M tokens. Source: plan 20260407-paperclip-cost-optimizations.md
const EQUIVALENT_PRICING: Record<string, { input: number; output: number; cached: number }> = {
...
function computeEquivalentApiCostCents(
  ...
  let pricing: { input: number; output: number; cached: number } | null = null;
  for (const family of Object.keys(EQUIVALENT_PRICING)) {
    if (lower.includes(family)) { pricing = EQUIVALENT_PRICING[family]!; break; }
  }
  if (!pricing) return 0;
  const usd =
...
export function heartbeatService(db: Db) {
  ...
  // PATCH 4.1: Ensure equivalent_api_cost_cents column exists on cost_events.
  // Idempotent — safe to run on every server start.
  void (async () => {
    try {
      await db.execute(sql`
        alter table cost_events
        add column if not exists equivalent_api_cost_cents integer not null default 0
      `);
      logger.info("patch4.1: ensured equivalent_api_cost_cents column on cost_events");
    } catch (err) {
      logger.warn({ err }, "patch4.1: failed to ensure equivalent_api_cost_cents column");
    }
  })();
  ...
  async function updateRuntimeState(
    ...
    if (additionalCostCents > 0 || hasTokenUsage) {
      ...
      // PATCH 4.1: Backfill equivalent_api_cost_cents on the row we just inserted.
      // For subscription_included rows cost_cents is 0 — this column lets the
      // dashboard show the equivalent dollar value without affecting budgets.
      try {
        const equivCents = computeEquivalentApiCostCents(
          result.model ?? null,
          inputTokens,
          outputTokens,
          cachedInputTokens,
        ...
        if (equivCents > 0) {
          await db.execute(sql`
            update cost_events
            set equivalent_api_cost_cents = ${equivCents}
            where id = (
              select id from cost_events
              where heartbeat_run_id = ${run.id}
        ...
        logger.warn({ err, runId: run.id }, "patch4.1: failed to update equivalent_api_cost_cents");
      }
    }
  }
    ...
    tickTimers: async (now = new Date()) => {
      ...
      try {
        ...
        for (const r of runningRuns) {
          ...
          const ag = allAgents.find((a) => a.id === r.agentId);
          let cap = DEFAULT_MAX_RUN_SEC;
          if (ag) {
            const rc = parseObject(ag.runtimeConfig);
            const hb = parseObject(rc.heartbeat);
            cap = Math.max(60, asNumber(hb.maxRunSec, DEFAULT_MAX_RUN_SEC));
          }
          if (ageSec > cap) {
            try {
              await cancelRunInternal(r.id, `Killed by janitor: exceeded ${cap}s timeout (ran ${Math.round(ageSec)}s)`);
              killedTimedOut += 1;
              logger.warn({ runId: r.id, agentId: r.agentId, ageSec, cap }, "Heartbeat janitor killed runaway run");
            } catch (err) {
              logger.error({ err, runId: r.id }, "Heartbeat janitor failed to cancel run");
            }
          }
        }
      } catch (err) {
      ...
      for (const agent of allAgents) {
        ...
        try {
          ...
          for (const r of recent) {
            const rj = parseObject(r.resultJson);
            const result = String(rj.result ?? "").toLowerCase();
            const isNoWork =
              result.includes("no assignment") ||
              result.includes("inbox empty") ||
              result.includes("nothing to do") ||
              result.includes("no_work_skipped");
            if (isNoWork) consecutiveNoWork += 1;
            else break;
          }
          if (consecutiveNoWork > 0) {
            const factor = Math.min(8, Math.pow(2, consecutiveNoWork));
            effectiveIntervalSec = policy.intervalSec * factor;
          }
        } catch (err) {
          logger.warn({ err, agentId: agent.id }, "Backoff calculation failed; using base interval");
        }

        if (elapsedMs < effectiveIntervalSec * 1000) continue;

        // ─── PATCH 1.1: Skip heartbeat when there is no actionable work ──
        try {
          const [{ count }] = await db
            .select({ count: sql<number>`count(*)` })
            .from(issues)
          ...
          const workCount = Number(count ?? 0);
          if (workCount === 0) {
            await db
              .update(agents)
          ...
          logger.warn({ err, agentId: agent.id }, "Pre-flight no-work check failed; spawning anyway");
        }
      ...
      return { checked, enqueued, skipped, skippedNoWork, killedTimedOut };
    },
