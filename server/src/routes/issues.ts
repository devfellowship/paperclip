...
export function issueRoutes(
  ...
  router.patch("/issues/:id", validate(updateIssueRouteSchema), async (req, res) => {
    ...
    // DEV-164: Deterministic guards (Harness v2 Fase 3)
    ...
    if (typeof updateFields.status === "string" && actor.actorType === "agent") {
      ...
      if (failedGuard) {
        ...
        void blockersSvc
          ...
          .catch((err: unknown) =>
            logger.warn({ err, issueId: existing.id }, "guard: blocker notification failed"),
          );
      }
    }
    let issue;
  ...
  router.post("/issues/:id/checkout", validate(checkoutIssueSchema), async (req, res) => {
    ...
    // DEV-251: Credential guard (Harness v2 WS-4)
    // After checkout succeeds, verify the assignee agent has all required credentials
    // for the task type. Missing creds → auto-block + Telegram via blockersSvc.
    if (req.actor.type === "agent") {
      try {
        ...
        if (!credCheck.ok) {
          ...
          await svc
            ...
            .catch((err: unknown) =>
              logger.warn({ err, issueId: updated.id }, "cred guard: failed to block issue"),
            );
          await svc
            .addComment(updated.id, blockedComment, {
              agentId: req.body.agentId,
              runId: checkoutRunId ?? undefined,
            })
            .catch((err: unknown) =>
              logger.warn({ err, issueId: updated.id }, "cred guard: failed to add comment"),
            );
          void blockersSvc
            .create({
              taskId: updated.id,
            ...
            .catch((err: unknown) =>
              logger.warn({ err }, "cred guard: blockersSvc.create failed"),
            );
